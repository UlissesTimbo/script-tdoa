...
% Lendo os sinais:
x1  = read_file_iq('teste15101g30.dat'); % IQ signal 1
x2  = read_file_iq('teste15103g30.dat'); % IQ signal 2
...
% subplot(2,2,1)
% plot(t/1e-3,real(x1))
% axis([0 0.5e-3/1e-3 1.1*min(real(x1)) 1.1*max(real(x1))])
% xlabel('t (ms)')
% ylabel('Real(x_1(t))')
% subplot(2,2,3)
% plot(t/1e-3,imag(x1))
...
% x1=x1-mean(x1);
% x2=x2-mean(x2);
x1=remove_mean(abs(x1));
x2=remove_mean(abs(x2));
